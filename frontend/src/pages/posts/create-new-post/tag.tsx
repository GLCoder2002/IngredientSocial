...
function Tags({setCategories, selectedKeys} : {
  setCategories : any;
  selectedKeys? : any
}) {
  ...
  useEffect(() => {
    ...
    //   await Http.get('/api/v1/category').then(res => {
    //     const categoryData = res.data.data.map((category : any) => ({
  ...
  return (<>
    ...
    <br/>
    <Form.Item label="Confirm Category" required>
      <Switch unCheckedChildren="Not Yet" checkedChildren="Confirm"
        checked={disabled}
